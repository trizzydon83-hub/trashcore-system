...
trashcore.downloadAndSaveMediaMessage(quoted, "gifee")
trashcore.sendMessage(from, {sticker:fs.readFileSync("/library/lin/media/gifee.webp")},{quoted:m})
} else if (/image/.test(mime)) {
let media = await quoted.download()
let encmedia = await trashcore.sendImageAsSticker(m.chat, media, m, { packname: pcknm, author: atnm })
} else if (/video/.test(mime)) {
if ((quoted.msg || quoted).seconds > 11) return reply('Maximum 10 Seconds!')
let media = await quoted.download()
let encmedia = await trashcore.sendVideoAsSticker(m.chat, media, m, { packname: pcknm, author: atnm })
} else {
...
await trashreply(`downloading song ${title}`);
            await trashcore.sendMessage(
                m.chat,
                {
